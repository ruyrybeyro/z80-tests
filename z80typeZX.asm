...
BORDCR	EQU	5C48h		; System variable holding the current border color
				; Only bits 4-6 are used to store the border color (0-7)
				; Can be read to get the current border or written to update 
...
; • No Ambiguity: Only two possible outcomes, eliminating user interpretation errors
; • Visual Certainty: Human eye easily distinguishes black from white border
...
; • Foolproof Design: Even if other bits affect other ULA functions, 
;   border color (bits 2-0) gives unambiguous visual indication
;
; TECHNICAL DETAILS:
...
; • No Restoration Needed: Border color change is cosmetic, not critical
;
...
TESTCMOSZX:
	...
	LD	C,0FEh		; ULA port address (controls border, speaker, etc.)
	...
	; 1   │ 0xFD │ A S D F G  
	; 2   │ 0xFB │ Q W E R T  ← W key here (bit 1)
	; 3   │ 0xF7 │ 1 2 3 4 5
	; 4   │ 0xEF │ 0 9 8 7 6
	; 5   │ 0xDF │ P O I U Y
	; 6   │ 0xBF │ Enter L K J H
	; 7   │ 0x7F │ Space SS M N B ← B key here (bit 4)
wait_key:
	; Check W key: Located in row 2 (Q-W-E-R-T row)
	ld   b,$FB		; port FBFE, line mask 11111101 (row 2 active)
				; Row mask 11111011b - selects row 2, bit 2=0
	...
	ld   b,$7F		; port 7FFE, line mask 11101111 (row 5 active)
				; Row mask 01111111b - selects row 5, bit 7=0
...
; • Register 1 (tone period) accepts any 8-bit value
...
; • Register 1 = Tone Generator B Fine Tune (8-bit, full range 0x00-0xFF)
...
TESTCMOSAY:
	ld bc,0FFFDh      	; AY register select port (address latch)
...
NMOS: 	
	...
	jr      nz,NORESTORE    ; If DEBUG is non-zero (Z=1), skip border restore
	...
	ld      (0FEh),a        ; Output the color in A to port FEh, restoring the border color
NORESTORE:
...
;
; CONTROL REGISTER BIT FUNCTIONS (Timex-specific):
; Bit 7-6: Video mode selection (0=standard, 1=hires, 2=multicolor, 3=512x192)
; Bit 5-4: Memory bank selection for upper 32K
; Bit 3-2: ROM selection (BASIC, DOS, cartridge)
; Bit 1-0: RAM/ROM mapping control
;
...
TESTCMOSTMX:
	...
	in a,(0ffh)		; Read current Timex control register value
	...
	in a,(0ffh)		; Read back the register value from latch
	...
	out (0ffh),a		; Write back original value to prevent system instability
...
; Tests voltage thresholds: NMOS ~0.3V, CMOS ~3.7V via bits 3&4 feedback
...
; easier to catch feeding values in the waiting loop
	...
	ld      bc,19FEh	; ULA - 25 tries for hysteresis
				; B=25 iterations for reliability, C=ULA port
LOOP:   
	DB      0EDH, 071H	; UNDOCUMENTED OUT (C),<0|0FFH> INSTRUCTION
				; NMOS: output 0x00, bits 3&4 = 00 (0.3V) → EAR bit 6 = 0
				; CMOS: output 0xFF, bits 3&4 = 11 (3.7V) → EAR bit 6 = 1
				; Voltage created by MIC+Speaker outputs affects EAR input

	DJNZ    LOOP		; Repeat 25 times to allow electrical settling/averaging

	; Read ULA register (includes EAR input at bit 6)
	in      a,(0feh)

	; Isolate EAR bit (bit 6) - affected by threshold voltage
	and     040h

	ld      a,0ffh		; Assume CMOS (high voltage detected)
	jr      nz,leaveout	; If bit 6 set (>threshold), return FFh (CMOS)
	xor     a		; Clear A for NMOS (low voltage, bit 6 clear)

leaveout:
	ret			; Return: A=0x00 (NMOS), A=0xFF (CMOS)
				; EXPERIMENTAL - may give inconsistent results!
...
ISAY:   
	LD      BC,0FFFDh	; BC = AY-3-8912 register select port 
				; (address latch)
	...
	JR      RTYPE2		; Skip remaining tests and return with result
...
ISTMX:  
	IN      A,(0ffh)	; A = current Timex control register value
	...
	OUT     (0ffh),A	; Write pattern to Timex control register
	IN      A,(255)		; A = readback from control register
	CP      03Ch		; Does it match what we wrote?
	...
	OUT     (0ffh),A	; Write second pattern to control register
	IN      A,(0ffh)	; A = readback from control register
	CP      0C3h		; Does it match what we wrote?
...
;------------------------------------------------------------------------------
; Cleanup and Return for Timex Detection
;------------------------------------------------------------------------------
RTYPE:  
	LD      A,B		; A = original Timex control register value
	OUT     (255),A		; Restore original value to prevent system instability
	JR	RTYPE2		; Jump to final return
...
HYSTE_TEST:
	...
	LD      BC,0FFFEh	; B=255 iterations, C=ULA port
HYST:
	OUT     (C),A		; Output voltage to ULA
				; Bit 3: speaker output level
				; Bit 4: MIC output level
				;	voltage high: bit 3,4=1
				;	voltage low : bit 3,4=0


	DJNZ    HYST		; Repeat to ensure voltage stabilizes
	IN      A,(C)		; A = ULA input register value
	POP     BC		; Restore main loop counter
	and     040h		; Check EAR input bit (bit 6)
	RET
...
HYSTEL1:
	...
	CALL	GETBORDER	; A = BORDER COLOUR (0-7)

				; Test low voltage output to EAR input
				; A = 0 (bit 3,4 low)
	CALL    HYSTE_TEST
	JR      nz,RTYPE2	; If high, hysteresis test failed

	; Test high voltage output to EAR input
	OR	14h		; A = 1 (bit 3,4 high)
	CALL	HYSTE_TEST
	JR	Z,RTYPE2	; If low, hysteresis test failed
...
;------------------------------------------------------------------------------
GETBORDER:
	LD      A,(BORDCR)      ; Load the current border color from the system variable BORDCR
                                ; BORDCR holds the border color in bits 4-6 (upper nibble)
	RRA                     ; Rotate A right through carry 3 times to move bits 4-6 into bits 0-2
	RRA                     ; (bit 4 → bit 1, bit 5 → bit 2, etc.)
	RRA                     ; After 3 rotations, bits 4-6 become bits 0-2
	AND	7		; Keep only bits 2,1,0 (0-7 black to white)
	RET                     ; Return with A containing the 3-bit border color (0-7)
...
MSGRUI		DB	' 2025 ZX Port Rui Ribeiro', 0DH
